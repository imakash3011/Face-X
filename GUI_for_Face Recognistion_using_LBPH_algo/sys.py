    ...
    if(is_number(Id) and name.isalpha()):
        ...
        while(True):
            ...
            for (x, y, w, h) in faces:
                ...
                cv2.imwrite(r"TrainImages\ " + name + "." + Id + '.' +
        ...
        row = [Id, name]
        with open(r"Records(Entries)\Entries.csv", 'a+') as csvFile:
...
def TrackImages():
    ...
    df = pd.read_csv(r"Records(Entries)\Entries.csv")
    cam = cv2.VideoCapture(0)
    bins = args['bins']
    # Initialize plot.
    fig, ax = plt.subplots()
    ...
    # Initialize plot line object(s). Turn on interactive plotting and show
    # plot.
    lw = 3
    ...
    while True:
        ...
        for(x, y, w, h) in faces:
            ...
            if(conf > 75):
                noOfFile = len(
                    os.listdir(r"C:\complete webdevelopment bootcamp\test1\ImageUnkown")) + 1
                cv2.imwrite(r"ImageUnkown\image/" +
                            str(noOfFile) + ".jpg", im[y:y + h, x:x + w])
            cv2.putText(im, str(tt), (x, y + h), font, 1, (255, 255, 255), 2)
            get_text = cv2.putText(
                im, str(tt), (x, y + h), font, 1, (255, 255, 255), 2)
    ...
    fileName = r"attendeance\attendeance/" + date + \
        "_" + Hour + "_" + Minute + "_" + Second + ".csv"
