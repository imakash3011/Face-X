...
while (True):
    ...
    for face in faces:
        try:
            ...
            bottom_right = (
                landmarks.part(13).x,
                landmarks.part(13).y + beard_ht)

            beard_gray = cv2.cvtColor(beard_adj, cv2.COLOR_BGR2GRAY)
            _, beard_mask = cv2.threshold(
                beard_gray, 25, 255, cv2.THRESH_BINARY_INV)
            beard_area = frame[top_left[1]: top_left[1] +
                               beard_ht, top_left[0]: top_left[0] + beard_wd]
            beard_area_no_beard = cv2.bitwise_and(
                beard_area, beard_area, mask=beard_mask)
            beard_final = cv2.add(beard_area_no_beard, beard_adj)
            frame[top_left[1]: top_left[1] + beard_ht,
                  top_left[0]: top_left[0] + beard_wd] = beard_final
            ...
            _, hat_mask = cv2.threshold(
                hat_gray, 30, 255, cv2.THRESH_BINARY_INV)
            hat_area = frame[top_left[1]: top_left[1] +
                             hat_ht, top_left[0]: top_left[0] + hat_wd]
            hat_area_no_hat = cv2.bitwise_and(
                hat_area, hat_area, mask=hat_mask)
            hat_final = cv2.add(hat_area_no_hat, hat_adj)
            frame[top_left[1]: top_left[1] + hat_ht,
                  top_left[0]: top_left[0] + hat_wd] = hat_final
